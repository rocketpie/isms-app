...
import {
  ...
  listLinkedData,
  unlinkData,
} from "@/lib/browser/isms/system-data";
...
export function useSystemData(systemId: string) {
  ...
  const link = useMutation({
    ...
    onMutate: async (dataId) => {
      ...
      const prev =
        queryClient.getQueryData<DataAssetView[]>(
    ...
    onError: (_e, _v, ctx) => {
      if (ctx?.prev)
        queryClient.setQueryData(queryKeys.systemData(systemId), ctx.prev);
    },
    onSettled: () => {
