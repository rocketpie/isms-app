...
export function useProcessApplications(processId: string) {
  ...
  const link = useMutation({
    mutationFn: (applicationId: string) => linkApplication(processId, applicationId),
    onMutate: async (applicationId) => {
      await queryClient.cancelQueries({ queryKey: queryKeys.processApplications(processId) });
      const prev = queryClient.getQueryData<ApplicationView[]>(queryKeys.processApplications(processId)) || [];
    ...
    onError: (_e, _v, ctx) => {
      if (ctx?.prev) queryClient.setQueryData(queryKeys.processApplications(processId), ctx.prev);
    },
    onSettled: () => {
  ...
  const unlink = useMutation({
    mutationFn: (applicationId: string) => unlinkApplication(processId, applicationId),
    onMutate: async (applicationId) => {
      await queryClient.cancelQueries({ queryKey: queryKeys.processApplications(processId) });
      const prev = queryClient.getQueryData<ApplicationView[]>(queryKeys.processApplications(processId)) || [];
      queryClient.setQueryData<ApplicationView[]>(
