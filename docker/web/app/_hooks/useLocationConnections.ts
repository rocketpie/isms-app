...
export function useLocationConnections(locationId: string) {
    ...
    const link = useMutation({
        mutationFn: (connectionId: string) => linkConnection(locationId, connectionId),
        onMutate: async (connectionId) => {
            await queryClient.cancelQueries({ queryKey: queryKeys.locationConnections(locationId) });
            const prev = queryClient.getQueryData<ConnectionView[]>(queryKeys.locationConnections(locationId)) || [];
        ...
        onError: (_e, _v, ctx) => {
            if (ctx?.prev) queryClient.setQueryData(queryKeys.locationConnections(locationId), ctx.prev);
        },
        onSettled: () => {
    ...
    const unlink = useMutation({
        mutationFn: (connectionId: string) => unlinkConnection(locationId, connectionId),
        onMutate: async (connectionId) => {
            await queryClient.cancelQueries({ queryKey: queryKeys.locationConnections(locationId) });
            const prev = queryClient.getQueryData<ConnectionView[]>(queryKeys.locationConnections(locationId)) || [];
            queryClient.setQueryData<ConnectionView[]>(
