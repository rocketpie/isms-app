...
export default function LinkedAssetSection<TChild extends BaseAssetView>(props: {
    className?: string,
    parentId: string,
    itemTypeName: string,
    ...
    const linkedItems = useMemo(() => props.linkHooks.listLinked.data ?? [], [props.linkHooks.listLinked.data]);
    const allItems = useMemo(() => props.linkHooks.listAll.data ?? [], [props.linkHooks.listAll.data]);
    ...
    return (
        <div className={props.className}>
            ...
            {(listLinked.error || listAll.error) && (
                <ErrorBanner message={((listLinked.error || listAll.error) as Error)?.message || 'Error'} />
            )}
            ...
            <ul className="grid gap-2">
                                ...
                                <div className="flex-1">
                                    {isEditing ? (
                                        <SimpleAssetEditorRow<TChild>
                                            value={value}
                                            owners={owners}
                                            disabled={props.assetHooks.update.isPending || props.assetHooks.remove.isPending}
                                            ...
                                            onDelete={() => {
                                                const ok = confirm(`Delete this ${props.itemTypeName}?\n\nNote: junctions may cascade.`);
                ...
                {(linkedItems).length === 0 && (
                    <li className="text-sm text-neutral-500">No {`${props.itemTypeName}`} linked.</li>
                )}
            </ul>
            ...
            <details className="group mt-2">
                ...
                <SimpleAssetCreateForm<TChild>
                    ...
                    className="mt-4"
                    onSubmit={(newItem) =>
                        props.assetHooks.create.mutateAsync(newItem)
                            .then((newId) => {
